...
import com.doublesymmetry.trackplayer.kotlinaudio.notification.NotificationManager
import com.doublesymmetry.trackplayer.kotlinaudio.players.components.PlayerCache
import com.doublesymmetry.trackplayer.kotlinaudio.utils.isUriLocal
...
internal constructor(
    ...
    private set(value) {
      if (value != field) {
        ...
        if (!playerConfig.handleAudioFocus) {
          when (value) {
            AudioPlayerState.IDLE, AudioPlayerState.ERROR -> abandonAudioFocusIfHeld()
  ...
  var ratingType: Int = RatingCompat.RATING_NONE
    set(value) {
      ...
      mediaSessionConnector.setRatingCallback(
          object : MediaSessionConnector.RatingCallback {
            ...
            override fun onSetRating(player: Player, rating: RatingCompat, extras: Bundle?) {
              playerEventHolder.updateOnPlayerActionTriggeredExternally(
                  MediaSessionCallback.RATING(rating, extras)
              )
            }
          }
      )
  ...
  init {
    ...
    val playerToUse =
        if (playerConfig.interceptPlayerActionsTriggeredExternally) createForwardingPlayer()
        else exoPlayer
  ...
  internal fun updateNotificationMetadataIfAutomatic() {
    if (automaticallyUpdateNotificationMetadata) {
      notificationManager.notificationMetadata =
          NotificationMetadata(currentItem?.title, currentItem?.artist, currentItem?.artwork)
    }
  }
  ...
  private fun getMediaItemFromAudioItem(audioItem: AudioItem): MediaItem {
    ...
    val mediaItem = 
      if (downloadedItem != null) {
        ...
        MediaItem.Builder().setUri(audioItem.audioUrl).setTag(AudioItemHolder(audioItem)).build()
      }
    return mediaItem
  }
  ...
  inner class PlayerListener : Listener {
    ...
    override fun onMetadata(metadata: Metadata) {
      PlaybackMetadata.fromId3Metadata(metadata)?.let {
        playerEventHolder.updateOnPlaybackMetadata(it)
      }
      PlaybackMetadata.fromIcy(metadata)?.let { playerEventHolder.updateOnPlaybackMetadata(it) }
      PlaybackMetadata.fromVorbisComment(metadata)?.let {
        playerEventHolder.updateOnPlaybackMetadata(it)
      }
      PlaybackMetadata.fromQuickTime(metadata)?.let {
        playerEventHolder.updateOnPlaybackMetadata(it)
      }
    }
     ...
     * to the current repeat mode. Note that this callback is also called when the playlist becomes
     * non-empty or empty as a consequence of a playlist change.
     */
    override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) {
      when (reason) {
        Player.MEDIA_ITEM_TRANSITION_REASON_AUTO ->
            playerEventHolder.updateAudioItemTransition(AudioItemTransitionReason.AUTO(oldPosition))
