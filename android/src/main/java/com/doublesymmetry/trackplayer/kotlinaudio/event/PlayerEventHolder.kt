package com.doublesymmetry.trackplayer.kotlinaudio.event
...
class PlayerEventHolder {
  ...
  /**
   * Use these events to track when
   * [com.doublesymmetry.trackplayer.kotlinaudio.players.BaseAudioPlayer.playWhenReady] changes.
   */
   ...
   *
   * Examples of an audio transition include changes to [AudioItem] queue, an [AudioItem] on repeat,
   * skipping an [AudioItem], or simply when the [AudioItem] has finished.
   */
   ...
   *
   * For this observable to send events, set [interceptPlayerActionsTriggeredExternally]
   * [com.doublesymmetry.trackplayer.kotlinaudio.models.PlayerConfig.interceptPlayerActionsTriggeredExternally]
   * to true.
   */
  ...
  internal fun updateOnPlaybackMetadata(metadata: PlaybackMetadata) {
    coroutineScope.launch { _onPlaybackMetadata.emit(metadata) }
  }
