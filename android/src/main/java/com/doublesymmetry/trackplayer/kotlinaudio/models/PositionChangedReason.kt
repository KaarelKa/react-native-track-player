...
/**
 * Use these events to track when and why the positionMs of an [AudioItem] changes. Examples include
 * changes to [AudioItem] queue, seeking, skipping, etc.
 */
sealed class PositionChangedReason(val oldPosition: Long, val newPosition: Long) {
  ...
  /**
   * Position has changed because a seek has occurred within the current [AudioItem], or another
   * one.
   */
