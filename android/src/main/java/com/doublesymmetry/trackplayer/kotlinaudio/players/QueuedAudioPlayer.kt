...
class QueuedAudioPlayer(
  ...
  override val currentItem: AudioItem?
    get() =
        (queue.getOrNull(currentIndex)?.mediaItem?.localConfiguration?.tag as AudioItemHolder?)
            ?.audioItem
  ...
  val previousItems: List<AudioItem>
    get() {
      return if (queue.isEmpty()) emptyList()
          ...
          queue.subList(0, exoPlayer.currentMediaItemIndex).map {
            (it.mediaItem.localConfiguration?.tag as AudioItemHolder).audioItem
          }
    }
  ...
  val nextItems: List<AudioItem>
    get() {
      return if (queue.isEmpty()) emptyList()
          ...
          queue.subList(exoPlayer.currentMediaItemIndex, queue.lastIndex).map {
            (it.mediaItem.localConfiguration?.tag as AudioItemHolder).audioItem
          }
    }
  ...
  fun jumpToItem(index: Int) {
    try {
      ...
      throw Error(
          "This item index $index does not exist. The size of the queue is ${queue.size} items."
      )
    }
  }

  /**
   * Replaces item at index in queue. If updating current index, we update the notification metadata
   * if [automaticallyUpdateNotificationMetadata] is true.
   */
