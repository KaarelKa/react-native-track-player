...
/**
 * Use these events to track when and why an [AudioItem] transitions to another. Examples of an
 * audio transition include changes to [AudioItem] queue, an [AudioItem] on repeat, skipping an
 * [AudioItem], or simply when the [AudioItem] has finished.
 */
sealed class AudioItemTransitionReason(val oldPosition: Long) {
   ...
   * [com.doublesymmetry.trackplayer.kotlinaudio.players.QueuedAudioPlayer.next] or
   * [QueuedAudioPlayer.previous]
   * [com.doublesymmetry.trackplayer.kotlinaudio.players.QueuedAudioPlayer.previous].
   */
  ...
  /**
   * The current [AudioItem] has changed because of a change in the queue. This can either be if the
   * [AudioItem] previously being played has been removed, or when the queue becomes non-empty after
   * being empty.
   */
